...
const milestoneDays = [0, 10, 20, 30] as const
...
function RecoveryChart({ service, compact = false }: { service?: Service; compact?: boolean }) {
  ...
  return (
    ...
    >
      <title id={titleId}>{label}, days 0 through 30</title>
      ...
      <g transform={`translate(${margin.left} ${margin.top})`}>
        ...
        {[0, 0.25, 0.5, 0.75, 1].map((tick) => {
          ...
          return (
            <g key={tick}>
              ...
              <text className="landing-chart-tick" x="-9" y={y + 3} textAnchor="end">{Math.round(tick * 100)}</text>
            </g>
          )
        })}
        {milestoneDays.map((day) => {
          const x = (day / 30) * plotWidth
          return <text className="landing-chart-tick" key={day} x={x} y={plotHeight + 20} textAnchor={day === 0 ? 'start' : day === 30 ? 'end' : 'middle'}>D{day}</text>
        })}
        {seriesOrder.map((key) => (
          <path
        ...
        {observationDays.map((day) => (
          <circle
            ...
            cx={(day / 30) * plotWidth}
            cy={plotHeight - historical[day] * plotHeight}
            key={day}
            r={compact ? 2.5 : 3.5}
          >
  ...
  return (
    <div className="landing-table-wrap hero-table-wrap">
      <table className="landing-table milestone-table">
        <caption>Overall recovery index at four milestones</caption>
        <thead>
  ...
  return (
    <section className="landing-section service-evidence" aria-labelledby="services-title">
      <header className="section-heading">
        ...
        <p>Each panel keeps the same zero-to-100 scale, so improvements and gaps can be compared without a shifting axis.</p>
      </header>
    ...
    <section className="benchmark-section" aria-labelledby="benchmark-title">
      <div className="landing-section benchmark-inner">
        <header className="section-heading benchmark-heading">
          ...
          <p>These are solved-case results from the synthetic 200-case benchmark. They are not values on the Hurricane Maria recovery index.</p>
        </header>
        <div className="landing-table-wrap benchmark-table-wrap">
          <table className="landing-table benchmark-table">
            <caption>Canonical final-split results for all seven evaluated planners</caption>
      ...
      <main>
        <section className="landing-hero" id="retrospective-evidence" aria-labelledby="landing-title">
          <div className="hero-intro">
            <p className="landing-kicker">Hurricane Maria · Puerto Rico · Sep 20–Oct 20, 2017</p>
            ...
            <div className="hero-contract" aria-label="Retrospective contract">
              <span><b>{mariaRetrospective.days.length}</b> dated points</span><span><b>{mariaRetrospective.serviceOrder.length}</b> service indices</span><span><b>1</b> frozen scenario</span>
            </div>
          </div>

          <div className="hero-evidence">
            <figure className="hero-chart-card">
              <figcaption><div><span>Figure 01 / derived index</span><h2>Recovery trajectory</h2></div><b>Day 0–30 · 0–100</b></figcaption>
              <RecoveryChart />
              <EvidenceLegend />
            </figure>
            <MilestoneTable />
        ...
        <section className="landing-final-cta" aria-labelledby="cta-title">
          ...
          <p>The Analyst Toolbox exposes the full 73-input, 22-action trace, paired shock tape, intervention ledger, and official outcome checks.</p>
