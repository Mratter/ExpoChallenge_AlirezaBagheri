...
# The comparison baseline is deliberately small and causal: a one-day GLOP
# program that sees exactly the current public state.  Version 2.1 retains the
# established single-slope plan and applies a fixed 15% correction toward a
# two-segment diminishing-return plan.  It does not inspect the precomputed
# shock tape or the PPO action.
BASELINE_V2_ID = "ortools-glop-visible-v2"
BASELINE_V2_VERSION = "2.1.0"
BASELINE_V2_UTILITY_SEGMENTS = 2
BASELINE_V2_DIMINISHING_RETURN_BLEND = 0.15
...
def ortools_proposal_v2(
    ...
    for index in range(5):
        ...
        width = max_dispatch / BASELINE_V2_UTILITY_SEGMENTS
        segments = [
            solver.NumVar(0.0, width, f"u_{SERVICES[index]}_{segment}")
            for segment in range(BASELINE_V2_UTILITY_SEGMENTS)
        ]
    ...
    for index, segments in enumerate(utility_segments):
        ...
        breakpoints = np.linspace(
            0.0, max_dispatch, BASELINE_V2_UTILITY_SEGMENTS + 1
        )
    ...
    single_dispatches = [
        single_solver.NumVar(0.0, single_solver.infinity(), f"single_y_{SERVICES[index]}")
        for index in range(5)
    ]
    ...
    for index in range(5):
        ...
        single_solver.Add(
            single_dispatches[index] <= single_allocations[index] + float(reserve[index])
        )
    ...
    if single_status != pywraplp.Solver.OPTIMAL:
        raise RuntimeError(f"OR-Tools v2 single-slope baseline failed with status {single_status}")
    single_slope_proposal = np.array(
        [variable.solution_value() for variable in single_allocations]
    )
    proposal = (
        (1.0 - BASELINE_V2_DIMINISHING_RETURN_BLEND) * single_slope_proposal
        + BASELINE_V2_DIMINISHING_RETURN_BLEND * diminishing_return_proposal
    )
    ...
    return proposal, {
        ...
        "utility_segments_per_service": BASELINE_V2_UTILITY_SEGMENTS,
        "diminishing_return_blend": BASELINE_V2_DIMINISHING_RETURN_BLEND,
        "single_slope_objective_coefficients": _round_vector(
            single_slope_coefficients
        ),
        "marginal_utility_slopes": [
            _round_vector(np.asarray(slopes, dtype=np.float64)) for slopes in marginal_slopes
        ],
        "single_slope_allocation_solution": _round_vector(single_slope_proposal),
        "diminishing_return_allocation_solution": _round_vector(
...
def compare_v2(scenario: Scenario, seed: int, policy_bundle: Any) -> dict[str, Any]:
    ...
    return {
        ...
        "baseline_spec": {
            ...
            "solver": "GLOP",
            "objective": (
                "established dispatch objective with a 15% correction toward a "
                "two-segment public-state diminishing-return repair utility"
            ),
            "utility_segments_per_service": BASELINE_V2_UTILITY_SEGMENTS,
            "diminishing_return_blend": BASELINE_V2_DIMINISHING_RETURN_BLEND,
            "future_shocks_visible": False,
