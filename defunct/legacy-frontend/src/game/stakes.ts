...
export type DisasterTally = {
  ...
  authored: number
  player: number
  storedUnknown: number
  total: number
}

export type ScheduleDisclosureStatus = 'reached' | 'not-reached' | 'overridden'

export type ScheduleDisclosureEntry = {
  day: number
  type: ShockType
  severity: number
  strongestService: Service | null
  source: 'Seeded ambient draw' | 'Player' | string
  status: ScheduleDisclosureStatus
}

export type DebriefShockProvenance = {
  authoredShocks?: readonly ForcedShock[]
  authoredLabel?: string
  playerShocks?: readonly ForcedShock[]
}

export const STORED_FORCED_EVENT_SOURCE = 'Stored forced event — origin unavailable'
...
export type RunDebrief = {
  ...
  schedule: ScheduleDisclosureEntry[]
  shockAdjustedArrivalShortfall: number
...
function sameForcedShock(
  left: Pick<ForcedShock, 'day' | 'type' | 'severity'>,
  right: Pick<ForcedShock, 'day' | 'type' | 'severity'>,
): boolean {
  return left.day === right.day
    && left.type === right.type
    && Math.abs(left.severity - right.severity) <= 1e-9
}

function strongestServiceForImpact(result: CompareResponse, impact: readonly number[]): Service {
  let index = 0
  impact.forEach((value, offset) => {
    if (value > (impact[index] ?? Number.NEGATIVE_INFINITY)) index = offset
  })
  return result.services[index] ?? 'public_services'
}

export function deriveRunDebrief(
  result: CompareResponse,
  provenance: DebriefShockProvenance = {},
): RunDebrief {
  ...
  const defaultAuthored = [
    ...(result.scenario.forced_shock ? [result.scenario.forced_shock] : []),
    ...(result.scenario.forced_shocks ?? []),
  ]
  const authoredShocks = provenance.authoredShocks ?? defaultAuthored
  const playerShocks = provenance.playerShocks ?? []
  const authoredLabel = provenance.authoredLabel ?? STORED_FORCED_EVENT_SOURCE
  const actualSchedule: ScheduleDisclosureEntry[] = result.shock_schedule.flatMap((shock) => {
    if (!shock.type) return []
    const candidateShock = { day: shock.day, type: shock.type as ShockType, severity: shock.severity }
    const source = playerShocks.some((entry) => sameForcedShock(entry, candidateShock))
      ? 'Player'
      : authoredShocks.some((entry) => sameForcedShock(entry, candidateShock))
        ? authoredLabel
        : shock.forced
          ? STORED_FORCED_EVENT_SOURCE
          : 'Seeded ambient draw'
    return [{
      ...candidateShock,
      strongestService: strongestServiceForImpact(result, shock.impact),
      source,
      status: shock.day <= terminalDay ? 'reached' as const : 'not-reached' as const,
    }]
  })
  const overriddenAuthored: ScheduleDisclosureEntry[] = authoredShocks.flatMap((authored) => {
    const actual = result.shock_schedule[authored.day - 1]
    const playerOverrodeDay = playerShocks.some((entry) => entry.day === authored.day)
    if (!playerOverrodeDay && actual?.type && sameForcedShock(authored, {
      day: actual.day,
      type: actual.type as ShockType,
      severity: actual.severity,
    })) return []
    return [{
      ...authored,
      strongestService: null,
      source: authoredLabel,
      status: 'overridden' as const,
    }]
  })
  const statusRank: Readonly<Record<ScheduleDisclosureStatus, number>> = {
    overridden: 0,
    reached: 1,
    'not-reached': 2,
  }
  const schedule = [...actualSchedule, ...overriddenAuthored].sort((left, right) => (
    left.day - right.day || statusRank[left.status] - statusRank[right.status]
  ))
  const endured = actualSchedule.filter((entry) => entry.status === 'reached')
  const player = endured.filter((entry) => entry.source === 'Player').length
  const storedUnknown = endured.filter((entry) => entry.source === STORED_FORCED_EVENT_SOURCE).length
  const authored = endured.filter((entry) => (
    entry.source !== 'Player'
    && entry.source !== 'Seeded ambient draw'
    && entry.source !== STORED_FORCED_EVENT_SOURCE
  )).length
  const disasters = {
    ambient: endured.filter((entry) => entry.source === 'Seeded ambient draw').length,
    authored,
    player,
    storedUnknown,
  ...
  return {
    ...
    schedule,
    shockAdjustedArrivalShortfall: shockAdjustedArrivalShortfall(result, candidate.conditions.length),
