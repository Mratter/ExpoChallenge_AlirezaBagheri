...
import { incidentRecoveryProgress, incidentRecoveryProgressAt, latestIncidentOfType } from './realism'
...
export type RoadNetworkState = {
  ...
  quakeProgress: number
  /** Continuous returned-footprint pressure remaining after presented transport recovery. */
  quakeLoad: number
  quakeHousingProgress: number
}

export function infrastructureRepairVisibility(
  state: Pick<RoadNetworkState, 'debrisSegments' | 'freshPatches'>,
  responseEnabled: boolean,
): { debrisSegments: number; freshPatches: number } {
  return responseEnabled
    ? { debrisSegments: state.debrisSegments, freshPatches: state.freshPatches }
    : { debrisSegments: 0, freshPatches: 0 }
}

export function roadNetworkState(
  ...
  const transportLevel = presentationServices?.[transportIndex] ?? day.services_end[transportIndex]
  const quakeIncident = latestIncidentOfType(result, dayIndex, 'aftershock')
  const weatherIncident = latestIncidentOfType(result, dayIndex, 'weather')
  const quakeProgress = quakeIncident
    ? presentationServices
      ? incidentRecoveryProgressAt(result, quakeIncident.dayIndex, dayIndex, 'transport', transportLevel)
      : incidentRecoveryProgress(result, quakeIncident.dayIndex, dayIndex, 'transport')
    : 1
  const housingLevel = presentationServices?.[result.services.indexOf('housing')]
  const quakeHousingProgress = quakeIncident
    ? housingLevel === undefined
      ? incidentRecoveryProgress(result, quakeIncident.dayIndex, dayIndex, 'housing')
      : incidentRecoveryProgressAt(result, quakeIncident.dayIndex, dayIndex, 'housing', housingLevel)
    : 1
  const weatherProgress = weatherIncident
    ? incidentRecoveryProgress(result, weatherIncident.dayIndex, dayIndex, 'transport')
    : 1
  const quakeAge = quakeIncident && quakeProgress < 1 ? quakeIncident.ageDays : null
  const weatherAge = weatherIncident && weatherProgress < 1 ? weatherIncident.ageDays : null
  const quakeLoad = quakeIncident && quakeProgress < 1
    ? quakeIncident.shock.severity
      * (quakeIncident.shock.impact[transportIndex] ?? 0)
      * (1 - quakeProgress)
    : 0
  let condition: RoadCondition = 'open'
  // Classification follows the continuous presented service and unresolved
  // typed footprint. `quakeAge` is retained only as exact incident metadata;
  // using its integer day bucket here made an unchanged road jump from
  // restricted to cracked at midnight.
  if (transportLevel < 0.24 || quakeLoad > 0.22) condition = 'closed'
  else if (transportLevel < 0.42 || quakeLoad > 0.13) condition = 'restricted'
  else if (transportLevel < 0.68 || quakeLoad > 0.001) condition = 'cracked'
  const closedSegments = condition === 'closed' ? 4 : condition === 'restricted' ? 2 : 0
  const debrisSegments = quakeAge === null
    ? 0
    : Math.max(1, Math.ceil((1 - quakeProgress) * 6))
  let freshPatches = 0
  for (let index = dayIndex; index >= Math.max(0, dayIndex - 4); index -= 1) {
    const repairDay = result.candidate.trajectory[index]
    const priorLevel = result.candidate.trajectory[index - 1]?.services_end[transportIndex]
      ?? repairDay.services_after_shock[transportIndex]
    // On the current visual day, use only the shared presentation level. A
    // shock day's pre-impact descent is above services_end, so it is not
    // mistaken for repair; all positive gains stay anchored to returned daily
    // endpoints and recorded repair supply.
    const recovered = index === dayIndex && presentationServices
      ? repairDay.shock.type && transportLevel > repairDay.services_end[transportIndex] + 1e-7
        ? 0
        : transportLevel - priorLevel
      : repairDay.services_end[transportIndex] - priorLevel
    const hasRecordedSupply = !repairDay.logistics || (repairDay.logistics.repair_supply[transportIndex] ?? 0) > 0.001
    if (recovered > 0.003 && hasRecordedSupply) {
      const age = dayIndex - index
      freshPatches = Math.max(1, Math.ceil(Math.min(5, recovered / 0.008) * (1 - age / 5)))
      break
    }
  }
  return {
    ...
    signalPower: presentationServices?.[civicIndex] ?? day.services_end[civicIndex],
    freshPatches,
    quakeAge,
    weatherAge,
    quakeProgress,
    quakeLoad,
    quakeHousingProgress,
  }
}
...
function BridgeChokepoint({ state }: { state: RoadNetworkState }) {
  const closed = state.condition === 'closed'
  return (
...
function DebrisClearance({ state, reducedMotion, presentationTime }: {
  ...
  useFrame(() => {
    ...
    const progress = reducedMotion
      ? 0.55
      : presentationMotionPhase(presentationTime, 0.12)
...
function TrafficSignals({ power, utilityFailure, reducedMotion, presentationTime }: {
  power: number
  ...
  useFrame(() => {
    ...
    lamps.current.children.forEach((child, index) => {
      ...
      if (!material) return
      const flicker = utilityFailure && !reducedMotion ? Math.sin(presentationTime * 11 + index) > 0.25 : false
...
function LightPolesAndMainBreak({ state, severity, reducedMotion, presentationTime }: {
  state: RoadNetworkState
  severity: number
  reducedMotion: boolean
  presentationTime: number
  ...
  useFrame(() => {
    ...
    const pulse = reducedMotion ? 0.6 : 0.55 + Math.sin(presentationTime * 3.5) * 0.2
    spray.current.scale.y = pulse
  })
  const toppled = state.quakeAge === null
    ? 0
    : Math.min(4, Math.ceil(severity * 10 * Math.max(0.18, 1 - state.quakeProgress)))
  const capped = state.quakeAge === null || state.quakeHousingProgress >= 0.58
  return (
...
export function InfrastructureScene({
  ...
  const state = useMemo(
    () => roadNetworkState(result, dayIndex, presentationServices),
    [dayIndex, presentationServices, result],
  )
  const repairs = infrastructureRepairVisibility(state, responseEnabled)
  const day = result.candidate.trajectory[dayIndex]
  const utilityIncident = latestIncidentOfType(result, dayIndex, 'utility')
  const utilityStillActive = utilityIncident
    ? incidentRecoveryProgress(result, utilityIncident.dayIndex, dayIndex, 'public_services') < 1
    : false
  const utilityFailure = day.shock.type === 'utility' || (utilityStillActive && state.signalPower < 0.58)
  const quakeIncident = latestIncidentOfType(result, dayIndex, 'aftershock')
  const severity = state.quakeAge === null ? 0 : quakeIncident?.shock.severity ?? 0
  return (
    <group name="state-derived-infrastructure">
      ...
      <DebrisClearance
        state={{ ...state, debrisSegments: repairs.debrisSegments }}
        reducedMotion={reducedMotion}
        presentationTime={presentationTime}
      />
      <FreshPatches count={repairs.freshPatches} />
      <TrafficSignals power={state.signalPower} utilityFailure={utilityFailure} reducedMotion={reducedMotion} presentationTime={presentationTime} />
      <LightPolesAndMainBreak state={state} severity={severity} reducedMotion={reducedMotion} presentationTime={presentationTime} />
    </group>
  )
}
