...
function persistentRepairRank(
  buildingIndex: number,
  serviceOffset: number,
  buildingCount: number,
): number {
  const baseRank = damageOrderForBuilding(buildingIndex, buildingCount)
  const districtShift = (serviceOffset * 7) % buildingCount
  return (baseRank - districtShift + buildingCount) % buildingCount
}

/**
 * Selects a small active-work cohort from a real positive trajectory change. The
 * rank is stable across days, so a damaged site keeps its crew through consecutive
 * repair ticks; a later site enters only after an earlier building becomes intact
 * or the returned allocation supports a larger simultaneous cohort.
 */
export function rebuildingCohortForDay(
  ...
  const allocationRank = [...day.allocation]
    .map((value, allocationIndex) => ({ value, allocationIndex }))
    .sort((left, right) => right.value - left.value || left.allocationIndex - right.allocationIndex)
    .findIndex((entry) => entry.allocationIndex === index)
  const requestedCount = Math.max(2, 6 - Math.max(0, allocationRank))
  ...
  return Array.from({ length: buildingCount }, (_, buildingIndex) => buildingIndex)
    ...
    .sort((left, right) => (
      persistentRepairRank(left, index, buildingCount)
        - persistentRepairRank(right, index, buildingCount)
    ))
...
function recoveryWindow(result: CompareResponse, dayIndex: number, serviceIndex: number): string {
  ...
  const days = Math.max(0, recoveryIndex - dayIndex)
  return `${Math.max(0, days - 1)}–${days + 1} DAYS`
}
...
function logisticsNarration(
  ...
  const scheduledNextDay = ledger.pending_next_day[serviceIndex] ?? 0
  const constrainedHeld = ledger.capacity_overflow[serviceIndex] ?? 0
  ...
  const damage = damagePenalty > 1e-7
    ...
    : ''
  return ` DEPOT LEDGER: ${landed.toFixed(1)} UNITS LANDED; ${stock.toFixed(1)}/${capacity.toFixed(0)} STOCK;${damage} ${Math.round(throughput * 100)}% EFFECTIVE THROUGHPUT; ${repairDispatch.toFixed(1)} DISPATCHED, ${usable.toFixed(1)} EFFECTIVE REPAIR-SUPPLY UNITS; ${constrainedHeld.toFixed(1)} UNITS IN THE CONSTRAINED DOCK QUEUE; ${scheduledNextDay.toFixed(1)} SCHEDULED OR HELD FOR DAY ${day.day + 1}.${transfer}${spoilage}`
}
...
export function relayNarration(result: CompareResponse, dayIndex: number): string {
  ...
  if (criticalIndex >= 0) {
    ...
    return `CRITICAL FLOOR BREACHED — ${SERVICE_LABELS[service].toUpperCase()}. RECORDED PLANNER ASSIGNMENT: ${day.allocation[criticalIndex].toFixed(1)} UNITS ${movement}.${logisticsNarration(result, day, criticalIndex)}`
  }
...
/**
 * The orb needs a brief field sitrep that leaves the city visible. The complete
 * derived operations note remains available in the Toolbox decision log.
 */
export function relayOrbNarration(result: CompareResponse, dayIndex: number): string {
  const day = result.candidate.trajectory[dayIndex]
  const previous = result.candidate.trajectory[dayIndex - 1]
  if (!day) return 'TRAJECTORY STANDING BY.'
  const allocationIndex = day.allocation.reduce(
    (best, value, index, values) => value > values[best] ? index : best,
    0,
  )
  const leadService = result.services[allocationIndex]
  const criticalIndex = day.services_end.findIndex((value) => value < 0.12)

  if (day.shock.type) {
    const service = strongestImpact(day)
    const index = result.services.indexOf(service)
    const throughput = day.logistics?.throughput_factor[index]
    const throughputClause = throughput === undefined
      ? ''
      : ` ${Math.round(throughput * 100)}% EFFECTIVE THROUGHPUT.`
    return `${RELAY_SHOCK_LABELS[day.shock.type as ShockType]} · RAW ${day.shock.severity.toFixed(2)}. ${SERVICE_LABELS[service].toUpperCase()} POINT OF DISTRIBUTION: ${day.allocation[index].toFixed(1)} UNITS ASSIGNED.${throughputClause}`
  }

  if (criticalIndex >= 0) {
    const service = result.services[criticalIndex]
    return `CRITICAL FLOOR · ${SERVICE_LABELS[service].toUpperCase()} ${(day.services_end[criticalIndex] * 100).toFixed(1)}%. ${day.allocation[criticalIndex].toFixed(1)} UNITS ASSIGNED.`
  }

  if (previous) {
    const gains = day.services_end.map((value, index) => value - previous.services_end[index])
    const gainIndex = gains.reduce((best, value, index, values) => value > values[best] ? index : best, 0)
    if (gains[gainIndex] > 0.002) {
      return `RECOVERY · ${SERVICE_LABELS[result.services[gainIndex]].toUpperCase()} +${(gains[gainIndex] * 100).toFixed(1)} POINTS. ${day.allocation[gainIndex].toFixed(1)} UNITS ASSIGNED.`
    }
  }

  return `DAY ${day.day} SITREP. ${SERVICE_LABELS[leadService].toUpperCase()} POINT OF DISTRIBUTION LEADS AT ${day.allocation[allocationIndex].toFixed(1)} UNITS; ALL ${day.available_budget.toFixed(1)} ARRIVAL UNITS ASSIGNED.`
}
