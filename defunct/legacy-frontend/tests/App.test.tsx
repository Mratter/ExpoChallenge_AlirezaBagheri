...
function day(dayNumber: number, resilience: number): DayResult {
  return {
    day: dayNumber,
    shock: dayNumber === 5
      ? { ...shock, day: 5, type: 'utility', severity: 0.26, impact: [0.3, 0.35, 0.45, 0.7, 1], budget_factor: 0.3, forced: true }
      : { ...shock, day: dayNumber },
...
describe('Analyst Toolbox', () => {
  ...
  it('presents the frozen aftershock key as Earthquake throughout visible evidence', async () => {
    const fixture = responseFixture()
    const earthquake: Shock = {
      day: 5,
      type: 'aftershock',
      severity: 0.26,
      impact: [0.65, 1, 0.2, 0.35, 0.45],
      budget_factor: 0.15,
      forced: true,
    }
    fixture.shock_schedule[4] = earthquake
    fixture.candidate.trajectory[4].shock = earthquake
    fixture.baseline.trajectory[4].shock = earthquake
    vi.stubGlobal('fetch', vi.fn().mockImplementation(async (input: RequestInfo | URL) => {
      if (String(input) === '/api/v1/simulations') return { ok: true, json: async () => ({ results: [] }) }
    ...
    expect(screen.getByText('Earthquake / 26.0%')).toBeVisible()
    expect(screen.getByText('Day 5: Earthquake, 26.0%, forced')).toBeInTheDocument()
    fireEvent.click(screen.getByRole('tab', { name: 'Daily audit' }))
    expect(screen.getByText('Earthquake (forced)')).toBeVisible()
  })
  ...
  it('implements keyboard navigation and labelled panels for result tabs', async () => {
    ...
    expect(screen.getByRole('tab', { name: 'v5 preview' })).toBeVisible()
    ...
    fireEvent.keyDown(auditTab, { key: 'End' })
    const realismTab = screen.getByRole('tab', { name: 'Realism ledger' })
    expect(realismTab).toHaveAttribute('aria-selected', 'true')
    expect(realismTab).toHaveFocus()
    expect(screen.getByRole('tabpanel', { name: 'Realism ledger' })).toBeVisible()

    fireEvent.keyDown(realismTab, { key: 'Home' })
    expect(trajectoryTab).toHaveAttribute('aria-selected', 'true')
    expect(trajectoryTab).toHaveFocus()
  })

  it('shows exact allocation-backed dispatches while leaving legacy depot operations unavailable', async () => {
    render(<App />)
    await screen.findByRole('heading', { name: 'Central district restart' })
    fireEvent.change(screen.getByLabelText('Inspect simulation day'), { target: { value: '1' } })
    fireEvent.click(screen.getByRole('tab', { name: 'Dispatch manifest' }))

    expect(screen.getByRole('tabpanel', { name: 'Dispatch manifest' })).toBeVisible()
    expect(screen.getByText('1 unit = 1 pallet.')).toBeVisible()
    const totals = screen.getByLabelText('Day 1 dispatch totals')
    expect(totals).toHaveTextContent('180.0 units = pallets')
    expect(totals).toHaveTextContent('60 load equivalents / day')

    const summary = screen.getByRole('table', { name: 'Day 1 dispatch summary by district depot' })
    expect(within(summary).getAllByText('36.0 assigned units')).toHaveLength(5)
    expect(within(summary).getAllByText('allocation-backed presentation; depot stock unavailable')).toHaveLength(5)
    expect(within(summary).getAllByText('Unavailable')).toHaveLength(10)
    expect(summary).not.toHaveTextContent('view-layer activity signal')
    expect(summary).not.toHaveTextContent('presentation-derived only')
    expect(within(summary).getAllByText('3 load equivalents')).toHaveLength(5)
    expect(within(summary).getAllByText('9 load equivalents')).toHaveLength(5)

    const exact = screen.getByRole('table', { name: 'Exact load-to-vehicle manifest for day 1' })
    expect(exact).toHaveTextContent('d1-inbound-0')
    expect(exact).toHaveTextContent('returns empty beyond the plate edge')
    expect(exact).toHaveTextContent('12.0 units = 12.0 pallets')
  })

  it('logs every candidate day with a data-derived sitrep, allocation, and rationale', async () => {
    render(<App />)
    await screen.findByRole('heading', { name: 'Central district restart' })
    fireEvent.click(screen.getByRole('tab', { name: 'Decision log' }))

    expect(screen.getByRole('tabpanel', { name: 'Decision log' })).toBeVisible()
    const log = screen.getByRole('table', { name: 'Daily candidate allocations and derived rationale' })
    expect(within(log).getAllByRole('row')).toHaveLength(15)
    expect(log).toHaveTextContent('DAY 1 SITREP — FULL ARRIVALS')
    expect(log).toHaveTextContent('DAY 5 SITREP — REDUCED ARRIVALS')
    expect(log).toHaveTextContent('ARRIVALS REDUCED — RESEQUENCING')
    expect(log).toHaveTextContent('TR 36.0')
    expect(log).toHaveTextContent('180.0 units')
  })

  it('renders the anti-fabrication ledger with source, derivation, and claim limits', async () => {
    render(<App />)
    await screen.findByRole('heading', { name: 'Central district restart' })
    fireEvent.click(screen.getByRole('tab', { name: 'Realism ledger' }))

    expect(screen.getByRole('tabpanel', { name: 'Realism ledger' })).toBeVisible()
    const ledger = screen.getByRole('table', { name: 'Visible behavior to simulation-data source mapping' })
    expect(within(ledger).getByRole('columnheader', { name: 'Source' })).toBeVisible()
    expect(within(ledger).getByRole('columnheader', { name: 'Derivation' })).toBeVisible()
    expect(within(ledger).getByRole('columnheader', { name: 'Does not mean' })).toBeVisible()
    expect(ledger).toHaveTextContent('Central intake freight')
    expect(ledger).toHaveTextContent('candidate.trajectory[day].available_budget')
    expect(ledger).toHaveTextContent('All available units are assigned; no carry-over or unassigned balance exists.')
    expect(within(ledger).getAllByRole('row')).toHaveLength(REALISM_LEDGER.length + 1)
  })
  ...
  it('labels prior evidence when the authored draft changes', async () => {
    ...
    expect(screen.getByLabelText('Comparison summary')).toBeVisible()
    expect(screen.getByRole('button', { name: 'Relay City view' })).toBeDisabled()
  })
  ...
  it('boots the game route into setup without a comparison and applies the selected preset on Start', async () => {
    ...
    expect(screen.getByRole('heading', { name: 'Run Relay City through a recovery scenario.' })).toBeVisible()
  ...
  it('switches from the Toolbox route to the WebGL-safe city route and back', async () => {
    ...
    fireEvent.click(screen.getByRole('button', { name: 'Relay City view' }))
