...
`scripts/evaluate.py` is the compact comparison runner. Read `build_cases`, `resolve_policy`, `rollout`, and `aggregate` in that order to see how a named 200-case development or final split and policy set become matched per-case rows and paired statistics. Learned-policy evaluation on the final roster remains owner-reserved, and the repository contains no learned-v4 final result. The fixed privileged oracle study is a narrow diagnostic exception: it does not load or evaluate a learned policy on final.
...
The oracle achieved **187 / 200** development solves and **182 / 200** final solves. Its matched development comparison with the shipped policy records **177 both solved, 1 policy-only, 10 oracle-only, and 12 neither**; the 10 oracle-only rows are the remaining provable headroom. The oracle sees the complete future shock tape, is not a submission baseline or model-selection input, and provides an anytime achieved lower bound rather than a proof of optimality. All oracle rows have zero hard violations and exactly `0.0` conservation residual. Normal runtime policies remain causal public-state planners, and no learned-v4 final evaluation was performed.
...
- `tests/test_train_policy.py`, `test_evaluate.py`, `test_build_development_baselines.py`, `test_development_evidence.py`, `test_headroom.py`, `test_run_oracle_study.py`, and `test_publish_oracle_study.py` cover the scientific tools and their current plus historical evidence.
